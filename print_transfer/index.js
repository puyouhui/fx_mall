...
readConfig().then((CONFIG) => {
  ...
  var server;
  if (useSSL) {
    const key = readFileSync(path.join(__dirname, './src/ssl.key'), 'utf-8');
    const cert = readFileSync(path.join(__dirname, './src/ssl.pem'), 'utf-8');
    ...
    const subjectAltName = certificate.extensions ?
      certificate.extensions.find(({ name }) => name === 'subjectAltName') :
      null;

    if (subjectAltName && subjectAltName.altNames) {
      const domains = subjectAltName.altNames.map(({ value }) => toUnicode(value));
      ipAddress = domains.map((value) => `https://${value}:${port}`).join('\n');
    } else {
      // 如果没有 subjectAltName，使用证书的 CN 或 IP 地址
      const cn = certificate.subject.getField('CN');
      if (cn && cn.value) {
        ipAddress = `https://${cn.value}:${port}`;
      } else {
        ipAddress = `https://${getIPAddress()}:${port}`;
      }
    }
  } else {
...
<head>
  <meta charset="UTF-8">
  <title>node-hiprint-transit</title>
  <style>
    body {
...
<body>
  <div class="container">
    ...
    <div class="info">
      <strong>说明:</strong><br>
      这是一个 WebSocket 中转服务，用于连接 vue-plugin-hiprint 和 electron-hiprint。<br>
      不能通过浏览器直接访问，需要通过 Socket.io 客户端连接。
    </div>
    <div class="info">
  ...
  io.on('connection', (socket) => {
    ...
    const allClients =
      Array.from(io.sockets.sockets.values()).filter(
        ({ handshake }) => handshake.query && handshake.query.client === 'electron-hiprint',
      ).length || 0;
    ...
    socket.on('printerList', (printerList) => {
      CLIENT.get(sToken)[socket.id] = Object.assign({},
        CLIENT.get(sToken)[socket.id], { printerList },
      );
    });
